...
- **저장**: 현재 UI 편집은 브라우저 메모리에 있으며 서버 저장/자동 저장/다중 사용자 기능은 없다. 새로고침·재접속은 테스터 기본값을 다시 계산하고 편집 내용을 복원하지 않는다. 도메인 저장소 인터페이스가 실제 DB 구현을 의미하지 않는다.
...
- **화면 기본값**: `createWorkflowConfiguration`의 수요·가격·비용·평일 11–21시·3회 반복·월 26일은 제품 흐름을 확인하기 위한 수정 가능한 예시다. 도메인 모델 자체의 중립 기본값과 구분한다. 샘플 테이블당 4석은 명시적 예시 설정이며 사람의 확인으로 표시하지 않는다. 도면·매핑 변경이나 새 도면 import 후에는 사용자 확인이 필요하다.
- **테스터 후보지**: 백초밥 명지점·백초밥·부산광역시 강서구 명지국제2로 80 비주거시설동 1층 1-86, 1-87호·94.44 m²는 사용자 제공 정보다. 기존 약 159.9 m² 공개 도면은 해당 점포의 실측 자료가 아니며 입력 면적에 맞춰 축척·정원을 변경하지 않는다. 상권·운영·가격·비용은 별도 가상 예시다.
- **테스터 시작**: 요청된 체험 모드에서는 입력 없이 모든 분석을 자동 준비한다. `sampleSpaceReady`와 실제 사용자 확인 상태를 분리한다. 입력을 편집하면 진행 중 계산을 취소하고 초기 결과가 사용자 값을 덮어쓰지 않도록 한다. 실제 공급자 장애 시 synthetic 자료로 자동 대체하는 경로는 없다.
...
- **최신성**: 입력이 바뀌면 관련 결과를 자동 갱신하며 그동안 이전 결과임을 표시한다. 입력이 같은 운영 기록은 재사용하고 배치·시각은 원래 snapshot을 유지한다. 엑셀 제안서는 계산 시점의 검토 자료이며 전체 프로젝트 복원 파일은 아니다.
