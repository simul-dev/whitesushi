...
이 문서는 Phase 0~8과 Delivery 확장의 아키텍처 및 분석 가정이다. 실제 도면의 높이·형상·재질 가정은 기존 [루트 assumptions.md](../assumptions.md)에 유지한다.

- **범위**: 공간 검토와 Market demo → 홀/배달 Demand → 공유 주방 DES → 반복실험·민감도·조건부 재무 계산 및 8단계 UI를 구현한다. 실제 시장 예측은 하지 않는다. DES의 revenue=0은 호환성 placeholder이며 FinancialEngine이 완료 처리량으로 별도 매출을 계산한다.
...
- **화면 기본값**: `createWorkflowConfiguration`의 수요·가격·비용·평일 11–21시·3회 반복·월 26일은 제품 흐름을 확인하기 위한 수정 가능한 예시다. 도메인 모델 자체의 중립 기본값과 구분한다. 샘플 테이블당 4석은 사용자 확인이 필요하고 다른 도면에 자동 적용하지 않는다.
- **재생**: 운영 화면은 첫 반복실험의 30초 간격 상태 기록을 보여준다. 반복 평균이나 보행 애니메이션이 아니며 30초 사이의 짧은 상태는 생략될 수 있다. 재생 속도·일시정지·화면 frame rate는 계산 결과에 영향을 주지 않는다.
- **최신성**: 입력이 바뀌면 관련 결과를 재계산 필요 상태로 표시한다. 이전 운영 기록의 배치·시각은 원래 snapshot을 유지한다. `store-review.json`은 검토 요약이며 전체 프로젝트 복원 파일은 아니다.
