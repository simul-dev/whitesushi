...
Phase 6~7의 배달 orders/hour와 객단가·비용·월 영업일은 명시적인 사용자/합성 fixture 가정이다. 기존 mock 상권 자료로 실제 배달 시장 규모나 플랫폼 수수료를 추정하지 않는다. 반복실험·재무 계산이 추가되어도 입력 자료의 synthetic 출처와 신뢰도 한계는 그대로 유지한다. 원본 도면은 좌석 정원·조리 능력·매출을 자동 증명하지 않는다.
