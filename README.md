...
**계약하기 전에 먼저 열어보세요.** 후보 점포의 도면·상권 가정·홀과 배달 운영·조건별 수익을 연결하는 웹 MVP입니다. Phase 8에서 기존 Phase 0~7 + Delivery 엔진을 **후보지 → 공간설계 → 상권분석 → 수요가정 → 가상영업 → 시나리오 → 수익성 → 출점검토**의 8단계 화면으로 연결했습니다. 첫 화면은 **백초밥 명지점 테스터 샘플**을 자동으로 계산하므로 입력 없이 전체 흐름을 체험할 수 있습니다. 실제 상권 API, 서버 저장, 계정은 없으며 자동 준비되는 상권도 Demo 자료입니다.
...
3. **시나리오·수익성·출점검토**로 이동해 이미 계산된 비교와 요약을 확인합니다. `검토 결과 저장`으로 `store-review.json`을 내려받을 수 있습니다.
4. 다른 조건을 시험하려면 각 단계의 값을 수정하고 관련 분석을 다시 실행합니다. 후보지 정보, 수요·운영·비용과 도면은 모두 변경할 수 있습니다.
...
입력을 바꾸면 진행 중 샘플 계산을 취소하고 관련 결과에 재계산 필요 상태를 표시합니다. 작업은 현재 브라우저 메모리에 있으며 새로고침·재접속은 테스터 기본값으로 다시 계산합니다. 편집 내용을 유지하려면 종료 전에 도면 JSON과 검토 자료를 저장하세요. 검토 자료 JSON은 요약 내보내기이며 전체 프로젝트를 다시 여는 백업 파일은 아닙니다.
