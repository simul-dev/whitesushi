...
**계약하기 전에 먼저 열어보세요.** 후보 점포의 도면·상권 가정·홀과 배달 운영·조건별 수익을 연결하는 웹 MVP입니다. 기존 도메인 엔진을 **후보지 → 공간설계 → 상권분석 → 수요가정 → 가상영업 → 수익성 분석 → 시나리오 비교 → 출점검토**로 연결했습니다. 백초밥 로고와 브랜드 색상을 사용하며, 첫 화면은 **백초밥 명지점 샘플**을 자동으로 계산하므로 입력 없이 전체 흐름을 체험할 수 있습니다. 실제 상권 API, 서버 저장, 계정은 없으며 자동 준비되는 상권도 Demo 자료입니다.
...
<http://127.0.0.1:5173>을 엽니다. 개발 서버는 로컬 루프백에만 바인딩됩니다. `strictPort: true`이므로 5173 포트가 사용 중이면 다른 포트로 자동 이동하지 않고 시작 오류를 표시합니다. 이미 이 프로젝트의 개발 서버가 실행 중이라면 기존 주소를 사용하세요.
...
1. 첫 화면에서 **샘플 영업 바로 보기**를 누릅니다. 준비 중에도 화면을 이동할 수 있고, 계산이 끝나면 상권·수요·가상영업·시나리오·민감도·수익성 결과가 함께 준비됩니다. 샘플 체험에는 직접 입력이나 공간 확인 체크가 필요하지 않습니다.
2. **가상영업**의 재생을 누릅니다. 첫 번째 반복실험의 30초 간격 운영 기록을 일시정지·시간 이동으로 살펴봅니다. 재생 속도는 계산 결과에 영향을 주지 않습니다.
3. **수익성 분석 → 시나리오 비교 → 출점검토**에서 비교와 요약을 확인하고 **엑셀 제안서 다운로드**로 로고와 서식을 포함한 `.xlsx` 보고서를 저장합니다.
4. 다른 조건을 시험하려면 값을 수정합니다. 입력을 마치면 관련 분석에 자동 반영되며 이전 단계로 돌아가 재계산할 필요가 없습니다. 새 도면을 불러오거나 공간을 편집한 경우에는 운영 역할과 정원을 먼저 확인합니다.

기본 점포명 **백초밥 명지점**, 브랜드 **백초밥**, 주소 **부산광역시 강서구 명지국제2로 80 비주거시설동 1층 1-86, 1-87호**, 입력 면적 **94.44 m²**는 사용자가 제공한 정보입니다. 상권·가격·비용·운영은 가상 예시이며, 연결된 **159.9 m² 공개 예시 도면은 명지점의 실측 도면이 아닙니다.** 입력 면적에 맞춰 도면이나 정원을 변경하지 않습니다. 실제 도면을 불러오거나 공간을 편집하면 운영 매핑·정원을 다시 확인해야 합니다.

입력을 바꾸면 이전 계산을 취소하고 관련 결과를 자동 갱신합니다. 비용만 바뀌면 완료된 운영 기록을 재사용합니다. 작업은 현재 브라우저 메모리에 있으며 새로고침·재접속은 샘플 기본값으로 시작합니다. 종료 전에 도면 JSON과 엑셀 제안서를 저장하세요. 엑셀은 계산 시점의 제안 자료이며 전체 프로젝트 복원 파일은 아닙니다. 화면 구성과 보고서 설명은 [영업용 검토 경험](docs/sales-experience.md)에 정리했습니다.
